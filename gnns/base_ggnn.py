    ...
    **kwargs
    ...
    if ggnn_impl == 'torch_geometric':
        return torch_geometric.nn.GatedGraphConv(
            out_channels=state_size, num_layers=num_layers, aggr=aggr, bias=bias, **kwargs)
    elif ggnn_impl == 'team2':
        return BaseGGNN(
            state_size=state_size, num_layers=num_layers, aggr=aggr, bias=bias, **kwargs)
    else:
...
class BaseGGNN(MessagePassing):
    def __init__(self, state_size: int, out_channels: int, num_layers: int, aggr: str = 'add',
        ...
        # edge_type_tensor should be of the type (e, D, D), where e is the total number of edge types
        # and D is the feature size
        self.edge_type_weight = Param(torch.zeros(total_edge_types, state_size, state_size), requires_grad=True)
        self.edge_type_bias = Param(torch.zeros(1, state_size), requires_grad=True)
    ...
    def forward(self, x: Tensor, edge_index: Adj,
        ...
        """
        edge_attr: tensor of size (n, e) - n is the number of edges, e is the total number of edge types
        """
    ...
    def message(self, x_j: Tensor, edge_attr: OptTensor):
        ...
        edge_attr: tensor of size (n, e) - n is the number of edges, e is the total number of edge types
        """
        # print(torch.bmm(torch.einsum("ab,bcd->acd", (edge_attr, self.edge_type_weight)), x_j.unsqueeze(-1)).size())
        return x_j if edge_attr is None else \
            torch.bmm(torch.einsum("ab,bcd->acd", (edge_attr, self.edge_type_weight)), x_j.unsqueeze(-1)).squeeze() + \
...
class BaseNodeSelectionGGNN(nn.Module):
    def __init__(self, state_size: int, out_channels: int, num_layers: int, aggr: str = 'add',
        ...
        self.ggnn = BaseGGNN(state_size=state_size, out_channels=out_channels, num_layers=num_layers, aggr=aggr,
                             bias=bias, total_edge_types=total_edge_types, **kwargs)
